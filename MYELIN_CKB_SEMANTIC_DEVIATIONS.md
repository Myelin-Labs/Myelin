...
| --- | --- | --- | --- |
| D-01 | `Script::hash` is a Myelin-local BLAKE3/domain-separated identity, not the CKB script hash. | `exec/src/celltx/types.rs` | Local registries use a stable Myelin identity. CKB projection uses the CKB Molecule script hash instead. These hashes must never be substituted for one another. |
| D-02 | `VmSemantics::MyelinExtended` exposes Myelin-only helper behavior. | `exec/src/vm/` | It exists for non-court workloads. Session and court paths force `CkbStrict`; the genesis-bound application profile separately commits whether strict spawn/IPC syscalls are required. Extended execution cannot support a CKB script-verification claim. |
