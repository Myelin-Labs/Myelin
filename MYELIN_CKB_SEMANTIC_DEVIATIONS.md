...
| D-06 | Finality is a configured closed validator set: static committee or finite-session Tendermint. | `consensus/src/lib.rs`, `consensus/src/tendermint.rs` | This supports benchmarks and controlled sessions. It is not Nakamoto consensus or permissionless L2 security. Tendermint implements proposal/prevote/precommit, locks, nil/round changes and equivocation rejection, while networking and permissionless membership remain outside Myelin. Both engines finalise the same canonical session block under distinct signature domains. |
