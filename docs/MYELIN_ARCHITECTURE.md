...
`myelin-session-producer` owns the operational policy for closing candidate transaction windows. It implements immediate transaction-driven production, fixed intervals with explicit empty-block opt-in, availability-started cumulative open windows, disabled automatic production, and serialised manual production. Its strict application-neutral config and reusable schedule let embedded hosts select timing without reimplementing it. Its transaction-source port uses reservations so failed finality or orderly shutdown returns uncommitted work. Count and byte limits are checked before the fixed candidate reaches a `CandidateCommitter`; that port must still execute, obtain and verify genesis-bound finality, and atomically commit through `myelin-session`. Production timing does not decide VM validity, create finality, or become part of application protocol identity.
