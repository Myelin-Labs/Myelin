...
`myelin-session` advances a durable session head only after deterministic execution and verification through its session-owned `FinalityVerifier` port. Session genesis commits the consensus kind, canonical validator/quorum configuration, compiled-in module descriptor, and WAL schema, so an empty or populated store cannot be reopened under a substituted authority set or codec. Each finalised record binds the canonical block, module commitment, consensus-owned canonical proof, and transaction bytes. The same store transaction advances the durable head, latest state checkpoint, and outbox. Live commit and recovery both recheck that the verifier returned the exact canonical block hash.

`myelin-session-store-rocksdb` is the production persistence implementation for this interface. It uses a versioned database schema, synchronous RocksDB WAL, optimistic head CAS, atomic block/latest-checkpoint/head/outbox commits, module/config/schema-bound consensus-round WAL, and durable network queues. Rolling checkpoints avoid storing a full state snapshot with every historical block. On restart, recovery streams and audits the finalised block-and-proof chain, restores the latest checkpoint, verifies its root against the durable head, and only then opens the writer. Full historical transaction replay is a future deep-audit mode.
